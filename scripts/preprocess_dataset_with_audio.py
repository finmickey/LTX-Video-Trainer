...
class DatasetPreprocessor:
    ...
    def preprocess(self, args: PreprocessingArgs) -> None:  # noqa: PLR0912
        ...
        if is_file:
            ...
            if dataset_path.suffix.lower() not in [".csv", ".json", ".jsonl"]:
                raise ValueError(
                    f"Dataset file must be CSV, JSON, or JSONL format: {dataset_path}"
                )
        else:
            ...
            if not dataset_path.exists() or not dataset_path.is_dir():
                raise FileNotFoundError(
                    f"Dataset path does not exist: {dataset_path} or is not a directory"
                )
            ...
            if not video_file.exists():
                raise FileNotFoundError(
                    f"Video paths file does not exist: {video_file}"
                )
        ...
        console.print(
            f"Number of batches: {len(dataloader)} (batch size: {args.batch_size})"
        )
    ...
    def _load_models(self, model_source: str, load_text_encoder_in_8bit: bool) -> None:
        ...
        with console.status(
            f"[bold]Loading models from [cyan]{model_source}[/]...", spinner="dots"
        ):
    ...
    def _create_dataloader(
        ...
        console.print(
            f"[bold green]✓[/] Dataset loaded with [bold]{len(dataset)}[/] items"
        )
