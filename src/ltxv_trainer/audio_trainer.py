...
from ltxv_trainer.audio_dataset import PrecomputedAudioDataset
...
class LtxvAudioTrainer:
    ...
    def _init_dataloader(self) -> None:
        ...
        if self._dataset is None:
            self._dataset = PrecomputedAudioDataset(
        ...
        data_loader = DataLoader(
            ...
            collate_fn=self._collate_variable_length_embeddings,
        )
    ...
    def _sample_videos(self, progress: Progress) -> list[Path] | None:
        ...
        if use_images:
            if len(self._config.validation.images) != len(
                self._config.validation.prompts
            ):
                raise ValueError(
    ...
    @staticmethod
    def _collate_variable_length_embeddings(batch):
        # Separate out audio conditions
        audio_data_list = []
        for item in batch:
            audio_data_list.append(item.pop("audio_conditions", None))

        # Collate everything else using default_collate
        collated = default_collate(batch)

        # Handle audio conditions (only if they exist)
        if any(audio_data_list):
            # Find max sequence length
            seq_lengths = [
                ad["audio_latents"].shape[0] for ad in audio_data_list if ad is not None
            ]
            max_seq_len = max(seq_lengths)
            batch_size = len(audio_data_list)
            embed_dim = audio_data_list[0]["audio_latents"].shape[1]
            ...
            for i, ad in enumerate(audio_data_list):
                if ad is None:
                    continue
                seq_len = ad["audio_latents"].shape[0]
                padded_embeddings[i, :seq_len] = ad["audio_latents"]
                attention_masks[i, :seq_len] = True

            collated["audio_conditions"] = {
